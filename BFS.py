...
#######################################################################"""
def search():
    frontier, reached, actions, expands = Queue(), dict(), list(), list()
    king = King(board.rows, board.cols)
    curr = State(board.start_pos)
    if board.map[curr.curr_row][curr.curr_col] == "G":
        return actions, 1
    frontier.put(curr)  #push initial state to frontier
    reached[curr]= True
    nodesExplored = 0
    while not frontier.empty(): #m
        ...
        for state in expand:   #b
            state.parent = curr
            if state in reached:
                continue
            if board.map[state.curr_row][state.curr_col] == "G":
                board.map[curr.curr_row][curr.curr_col] = "."
                board.map[state.curr_row][state.curr_col] = '♚'
                while state.parent is not None:
                    temp1 = []
                    temp = ActionList(temp1)
                    temp.append(state.parent)
                    temp.append(state)
                    actions.append(temp)
                    state = state.parent
                board.print(True)
...
# TODO return: List of moves and nodes explored
def run_BFS():
    # You can code in here but you cannot remove this function or change the return type
    temp = list()
    moves = ActionList(temp)
    moves.list, nodesExplored = search() #For reference
